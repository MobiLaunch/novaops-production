      ...
      >
        <CardContent class="p-6">
          <div class="flex items-start justify-between mb-3">
            ...
            <div class="text-3xl font-bold">{{ activeTickets }}</div>
          </div>
          <div>
...
// Import icons with NO conflicts
import { 
  Banknote, 
  TicketCheck,
  Users, 
...
const { customers, tickets, inventory, appointments, settings, saveAll, trackDevice } = appStore
...
const totalRevenue = computed(() => tickets.value.reduce((sum, ticket) => sum + (ticket.price || 0), 0))
const activeTickets = computed(() => tickets.value.filter(t => t.status !== 'Completed' && t.status !== 'Delivered').length)
...
const lowStockItems = computed(() => inventory.value.filter(item => item.stock <= item.low).length)
